...
function widget_tag($title){
	...
	<div id="blogtags">
	<?php 
shuffle($tag_cache);$tag_cache = array_shift(array_chunk($tag_cache,40)); // 标签数量限制为40
...
function widget_twitter($title){
	...
	<ul id="twitter">
	<?php 
	shuffle ($newtws_cache);$newtws_cache = array_shift(array_chunk($newtws_cache,1)); // 微语数量限制为随机1条
...
function neighbor_log($neighborLog){
	extract($neighborLog);?>
		<nav id="nav-fixed">
	<?php if($prevLog):?>
	<div class="nav-previous"><a href="<?php echo Url::log($prevLog['gid']) ?>"><span class="meta-nav-prev"></span></a></div>
	<?php endif;?>
	<?php if($nextLog && $prevLog):?>
	<?php endif;?>
	<?php if($nextLog):?>
		 <div class="nav-next"><a href="<?php echo Url::log($nextLog['gid']) ?>"><span class="meta-nav-next"></span></a></div>
	<?php endif;?>
	</nav>
<?php }?>
...
<?php
//blog：相邻文章
function neighbor($neighborLog){
	extract($neighborLog);?>
	<nav id="nav-single">
	<?php if($prevLog):?>
	<span class="nav-prev"><a href="<?php echo Url::log($prevLog['gid']) ?>"><span class="meta-nav-prev">Prev Post</span></a></span>
	<?php endif;?>
	<?php if($nextLog && $prevLog):?>
	<?php endif;?>
	<?php if($nextLog):?>
		 <span class="nav-nexted"><a href="<?php echo Url::log($nextLog['gid']) ?>"><span class="meta-nav-next">Next Post</span></a></span>
	<?php endif;?>
	</nav>
<?php }?>
...
function blog_comments($comments){
    ...
    if($commentStacks): ?>
	<a name="comments"></a>
	<p class="comment-header"><b>HERE：</b></p>
	<?php endif; ?>
	<?php
	...
	<div class="comment" id="comment-<?php echo $comment['cid']; ?>">
		...
		<div class="comment-info">
			<div class="poster"><?php echo $comment['poster']; ?> </div><div class="comment-time"><?php echo $comment['date']; ?></div>
			<div class="comment-content"><?php echo $comment['content']; ?></div>
			<div class="comment-reply"><a href="#comment-<?php echo $comment['cid']; ?>" onclick="commentReply(<?php echo $comment['cid']; ?>,this)">Reply</a></div>
		</div>
...
function blog_comments_children($comments, $children){
	...
	<div class="comment-children" id="comment-<?php echo $comment['cid']; ?>">
		...
		<div class="comment-info">
			<div class="poster"><?php echo $comment['poster']; ?> </div><div class="comment-time"><?php echo $comment['date']; ?></div>
			<div class="comment-content"><?php echo $comment['content']; ?></div>
			<?php if($comment['level'] < 10): ?><div class="comment-reply"><a href="#comment-<?php echo $comment['cid']; ?>" onclick="commentReply(<?php echo $comment['cid']; ?>,this)">Reply</a></div><?php endif; ?>
		</div>
		<?php blog_comments_children($comments, $comment['children']);?>
	</div>
	<?php endforeach; ?>
...
function blog_comments_post($logid,$ckname,$ckmail,$ckurl,$verifyCode,$allow_remark){
	...
	<div class="comment-post" id="comment-post">
		...
		<p class="comment-header"><b>Leave a Reply：<br />Your email address will not be published. Required fields are marked *</b><a name="respond"></a></p>
		<form method="post" name="commentform" action="<?php echo BLOG_URL; ?>index.php?action=addcom" id="commentform">
			...
			<?php endif; ?>
			<p class="comment-header"><textarea name="comment" id="comment" rows="10" tabindex="4"></textarea></p>
			<p><?php echo $verifyCode; ?> <input type="submit" id="submit" value="Post Comment" tabindex="6" /></p>
...
<?php
//blog：自定义分页函数——pre
function my_page_pre($count, $perlogs, $page, $url, $anchor = '') {
...
<nav id="nav-fixed">
<div class="nav-previous"><a href="<?php echo $re ?>"><span class="meta-nav-prev"></span></a></div>
</nav>
 <?php } ?>

<?php
//blog：自定义分页函数——next
function my_page_next($count, $perlogs, $page, $url, $anchor = '') {
 $pnums = @ceil($count / $perlogs);
 $re = '';
 $urlHome = preg_replace("|[\?&/][^\./\?&=]*page[=/\-]|", "", $url);
 if($page < $pnums) {
...
<nav id="nav-fixed">
<div class="nav-next"><a href="<?php echo $re ?>"><span class="meta-nav-next"></span></a></div></nav>
 <?php } ?>